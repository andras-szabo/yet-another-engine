...
| P2-05 — Scene class + data-oriented hierarchy | ✅ Done | `Scene.ixx` (inside `namespace Engine::Scene`) implements `Scene : IScene` via a pimpl (`SceneImpl`). Parallel flat arrays: `_nodes`, `_hierarchy` (parent/firstChild/firstSibling/depth/isDirty), `_localTransforms`, `_globalTransforms`, `_nodeNames`, `_nodeTransformComponents`. `AddNode` appends to all arrays and links into the sibling-list hierarchy. `SetParent` unlinks from the old parent chain, re-links under the new parent, and recursively updates depths. `UpdateNodeIndex` moves the `Transform*` entry and calls `OnSceneNodeIndexChanged`. `IScene` is a separate module (`IScene.ixx`) imported by both `Scene` and `Transform`, breaking any circular dependency. `EngineInstance` holds a non-owning `IScene*` (ownership TBD) and exposes `GetActiveScene()`. |
| P2-06 — Scene graph traversal | ✅ Done | `SceneImpl` provides `WalkDepthFirst` (recursive, pre-order) and `WalkBreadthFirst` (iterative, using a persistent `std::deque<int> _walkHelperQueue` to reuse heap allocation). Both thread a `Scene&` reference through so callbacks can inspect the scene. `UpdateWorldTransforms` walks depth-first and recomputes `_globalTransforms[i] = _globalTransforms[parent] * _localTransforms[i]` for dirty nodes; root node (parent == -1) uses its local transform directly. |
