...
| P4-06 — Standalone game-template | ✅ Done | **(a)** `engine-core/CMakeLists.txt` gains `install()` rules producing `sdk/bin/<Config>/engine-core.dll`, `sdk/lib/<Config>/engine-core.lib`, `sdk/modules/*.ixx` (interface files only), `sdk/include/` (headers + `engine_core_api.h`), `sdk/cmake/CompilerOptions.cmake`. Run `cmake --install build --config Debug --prefix <path>` to produce the SDK. **(b)** `game-template/CMakeLists.txt` is now a standalone root CMake project accepting `ENGINE_CORE_SDK_DIR`; creates an IMPORTED SHARED target for engine-core, adds all SDK `.ixx` files to its own `FILE_SET CXX_MODULES` (MSVC recompiles them locally for `.ifc` generation), and links against the pre-built `.lib`. `/FORCE:MULTIPLE` suppresses duplicate-symbol linker errors caused by non-trivial function bodies remaining in some engine `.ixx` files; the critical singletons (`GlobalLoggerInstance`, `GlobalComponentRegistry`) and GUID's RNG are moved to `.cpp` module implementation units so they always resolve to the DLL. **(c)** `game-template` removed from the engine root `CMakeLists.txt`. **(d)** `SpinnerComponent` added in `game-template/src/` with `speed` (float), `axis` (Vec3), `active` (bool) reflected fields and full lifecycle hooks — reference implementation for game developers. **(e)** Full two-step workflow documented in `docs/Standalone-Game-Template.txt`. **Known limitations:** remaining `.ixx` method bodies (Scene, DataFile, GameObject, Transform) are a future refactor; pre-built `.ifc` distribution deferred until CMake support matures. |
...
- P4-06: Restructure game-template into a standalone CMake project that consumes pre-built engine-core artifacts (no engine source required), and add a sample custom component with reflected fields as a reference/starting point. Sub-tasks:
  - (a) Add a CMake install step to engine-core that produces an engine SDK: `engine-core.dll`, `engine-core.lib`, all `.ixx` module interface files, all `include/*.h` headers, and the generated `engine_core_api.h`. This is the only artifact the game developer needs.
  - (b) Give `game-template` its own root `CMakeLists.txt` (`cmake_minimum_required` + `project(GameTemplate)`). It accepts `ENGINE_CORE_SDK_DIR` as a CMake variable, creates an IMPORTED SHARED target for engine-core, adds the SDK's `.ixx` files to its own `FILE_SET CXX_MODULES` (MSVC recompiles the interfaces locally but links against the pre-built `.lib`), and sets include paths from the SDK. Note: distributing pre-compiled `.ifc` files is the cleaner long-term solution but CMake support for consuming pre-built module interfaces is not yet mature.
